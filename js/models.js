...
define(["knockout"], function (ko) {
    ...
    var KoModels = {
        FareMatrix: function (data) {
            this.MatrixID = ko.observable(data.MatrixID || 0).extend({numeric: 0});
            this.MinimumFare = ko.observable(data.MinimumFare || 0.00).extend({numeric: 2});
            this.SucceedingRate = ko.observable(data.SucceedingRate || 0.00);
        },
        RouteStop: function (data) {
            this.ID = ko.observable(data.ID || 0).extend({numeric: 0});
            this.Name = ko.observable(data.Name || "");
        },
        JeepRoute: function (data) {
            var self = this;
            self.RouteID = ko.observable(data.RouteID || 0).extend({numeric: 0});
        ...
        Jeep: function (data) {
            this.JeepID = ko.observable(data.JeepID || 0).extend({numeric: 0});
            this.PlateNo = ko.observable(data.PlateNo || "xxx123");
            this.PaxCapacity = ko.observable(data.PaxCapacity || 0).extend({numeric: 0});
            this.RouteID = ko.observable(data.RouteID || 0).extend({numeric: 0});
        },
        Pasada: function (data) {
            this.PasadaID = ko.observable(data.PasadaID || 0).extend({numeric: 0});
        ...
        Bayad: function (data) {
            var self = this;
            self.BayadID = ko.observable(data.BayadID || 0).extend({numeric: 2});
            self.PasadaID = ko.observable(data.PasadaID || 0).extend({numeric: 2});
